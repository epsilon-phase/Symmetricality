...
#include <cmath>
#include <iostream>
RadialMenu::RadialMenu() { continuation.loadFromFile("menu_continuance.png"); }

RadialMenu::~RadialMenu() {}
void RadialMenu::addItem(const sf::Texture &t, sf::IntRect texrect,
  ...
  if (displayables.size() >= max * menu_added_artificially) {
    ...
    actions.push_back([=]() { this->increaseStart(); });
...
void RadialMenu::draw(sf::RenderTarget &target, sf::RenderStates) const {
  if (closed)
    return;
  int c_offset =
      opening ? rect_size - (rect_size - since_opened / open_rate) : rect_size;
  since_opened++;
...
bool RadialMenu::handle_event(sf::Event evt, sf::Vector2f coord) {
  if (closed)     // If the menu is closed, no event shall be handled.
    return false; // this means that the event shall continue into the rest of
                  // the Planrenderer input method
  if (evt.type ==
      sf::Event::MouseMoved) { // for updating the selection for drawing.
    selected = intersects(coord);
    /*The menu does not block other events from happening when the mouse is
     * moved and it is open.
     */
    return false;
  }
  if (evt.type == sf::Event::MouseWheelMoved &&
      intersects(sf::Vector2f(evt.mouseWheel.x, evt.mouseWheel.y))) {
  ...
  if (evt.type ==
      sf::Event::MouseButtonPressed) { // for menu interaction and closing.
    if (evt.mouseButton.button == 0) {
      ...
      return true; // This means that the PlanRenderer input method shall not
                   // proceed in handling this one event.
    }
  }
  return false;
}
int RadialMenu::intersects(sf::Vector2f f) const {
  for (unsigned int i = 0; i < displayables.size(); i++) {
    if (i >= start_index && displayables[i].getGlobalBounds().contains(f)) {
      return i;
      break;
    }
  }
...
void RadialMenu::close() { closed = true; }
