  ...
  if (event.type == sf::Event::KeyPressed) {
    ...
    case sf::Keyboard::F5:
      ...
      } else*/
      {
        ...
        std::string file_path =
            default_file_path +
            (event.key.shift ? "/blueprint.csv" : "/blueprint.ser");
        const char *filename = tinyfd_saveFileDialog(
            "Save File", file_path.c_str(), 1, &filters,
    ...
    case sf::Keyboard::F6: {
      ...
      const char *filename = tinyfd_openFileDialog(
          "Load blueprint", file_path.c_str(), 1, &filters, "None", 0);
      if (filename) {
        this->save_file = std::string(filename);
...
void Hud::update_text() {
  ...
  if (old_cursor != renderer.cursorpos || old_floor != renderer.Floornum) {
    ...
    if (renderer.blueprint.isDesignating()) {
      f << " "
        << std::abs(renderer.blueprint.getDesignationStart().x -
                    renderer.cursorpos.x)
        << "x"
        << std::abs(renderer.blueprint.getDesignationStart().y -
                    renderer.cursorpos.y)
        << "x"
        << std::abs(renderer.blueprint.getDesignationStart().z -
                    renderer.Floornum);
    }
...
void Hud::save_screenshot(const sf::Image &that) {
  std::stringstream a;
  a << save_file.substr(0, save_file.size() - 4) << "-" << screencap_no
    << ".png";
  screencap_no++;
  that.saveToFile(a.str());
}

void Hud::set_default_file_path(const std::string &s) { default_file_path = s; }
