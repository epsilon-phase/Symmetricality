...
PlanRenderer::PlanRenderer() {
  ...
  // This was done to allow visual studio to compile. The vile thing doesn't
  // allow non static initializers.
...
void PlanRenderer::draw(sf::RenderTarget &target,
...
void PlanRenderer::LoadConfiguration(Json::Value &v) {
  ...
  for (auto &f : b.second) {
    build_menu.addItem(*b.first, f.second.getTextureRect(),
                       [=]() { this->setBuilding(f.first); });
...
void PlanRenderer::set_pos(sf::Vector2i i, char i1) { build_vertex_array(); }
...
void PlanRenderer::handle_event(sf::Event event) {
  if (event.type == sf::Event::KeyPressed) {
    ...
    case sf::Keyboard::Space:
      if (building_mode) {
        blueprint.placeBuilding(cursorpos.x, cursorpos.y, Floornum,
                                current_building->second);
        building_changed = true;
      } else {
        designation_changed = true;
        blueprint.insert(cursorpos.x, cursorpos.y, Floornum,
                         *current_designation);
      }
      break;
    ...
    case sf::Keyboard::Return:
      ...
      blueprint.setDesignationToggle(
          cursorpos.x, cursorpos.y, Floornum,
          event.key.shift
              ? Blueprint::CIRCLE
              : (event.key.control ? Blueprint::LINE : Blueprint::RECTANGLE),
          building_mode);
      designation_changed = true;
      break;
...
void PlanRenderer::move_cursor(int x, int y) {
  ...
  if (transformed.x > viewable_area.getSize().x / 2) {
    ...
    if (transformed.x <
        viewable_area.getCenter().x - viewable_area.getSize().x / 2)
      move(m_square_size * std::abs(x), 0);
  }
  if (transformed.y > viewable_area.getSize().y / 2) {
    ...
    if (transformed.y <
        viewable_area.getCenter().y - viewable_area.getSize().y / 2)
      move(0, m_square_size * std::abs(y));
  }
  build_vertex_array();
}
...
void PlanRenderer::add_symmetry(Symmetry::Symmetry_Type type) {
  ...
  facets.back()->UpdateFromBlueprint(blueprint);
}
...
void PlanRenderer::build_designation() {
  if (blueprint.isDesignating()) {
    ...
    the_square[0].position = sf::Vector2f((0.5f + start.x) * m_square_size,
                                          (0.5f + start.y) * m_square_size);
    the_square[1].position = sf::Vector2f((0.5f + end.x) * m_square_size,
                                          (0.5f + start.y) * m_square_size);
    the_square[2].position = sf::Vector2f((0.5f + start.x) * m_square_size,
                                          (0.5f + start.y) * m_square_size);
    the_square[3].position = sf::Vector2f((0.5f + start.x) * m_square_size,
                                          (0.5f + end.y) * m_square_size);
    the_square[4].position = sf::Vector2f((0.5f + end.x) * m_square_size,
                                          (0.5f + start.y) * m_square_size);
    the_square[5].position = sf::Vector2f((0.5f + end.x) * m_square_size,
                                          (0.5f + end.y) * m_square_size);
    the_square[6].position = sf::Vector2f((0.5f + start.x) * m_square_size,
                                          (0.5f + end.y) * m_square_size);
    the_square[7].position = sf::Vector2f((0.5f + end.x) * m_square_size,
                                          (0.5f + end.y) * m_square_size);
    for (int i = 0; i < 8; i++)
...
void PlanRenderer::handle_mouse(sf::Event event, const sf::Vector2f &b) {
  using std::floor;
  if (!building_mode && menu.handle_event(event, b))
    return;
  if (building_mode && build_menu.handle_event(event, b))
    return;
  auto transformed = getInverseTransform().transformPoint(b);
  sf::Vector2i mouse_position =
      sf::Vector2i((int)floor(transformed.x / m_square_size),
...
void PlanRenderer::handleMouseOver(const sf::Vector2f &relative_to_view) {
  sf::Vector2f transformed_point =
      getInverseTransform().transformPoint(relative_to_view);
  if (right_button_down) {
    this->move(relative_to_view.x - old_mouse_pos.x,
               relative_to_view.y - old_mouse_pos.y);
    old_mouse_pos = relative_to_view;
  }
  sf::Vector2i mouse_position =
      sf::Vector2i((int)floor(transformed_point.x / m_square_size),
...
void PlanRenderer::buildCursorArray() {
  ...
  current[1].position = sf::Vector2f(
      current[0].position.x + .5f * m_square_size, current[0].position.y);
  current[2].position = sf::Vector2f(
      current[1].position.x, current[1].position.y + 0.5f * m_square_size);
  current[3].position = sf::Vector2f(
      current[2].position.x - 0.5f * m_square_size, current[2].position.y);
  for (int i = 0; i < 4; i++)
    current[i].color =
        building_mode && !blueprint.canPlace(cursorpos.x, cursorpos.y, Floornum,
                                             current_building->second)
...
void PlanRenderer::initializeMenu() {
  ...
  menu.addItem(menu_utility_texture,
               sf::IntRect(0, 0, menu_tex_width, menu_tex_height), [=]() {
                 const char *filters = "*.ser";
                 std::string file_path =
                     the_hud->default_file_path + "/blueprint.ser";
                 const char *filename =
                     tinyfd_saveFileDialog("Save File", file_path.c_str(), 1,
  ...
  menu.addItem(
      menu_utility_texture,
      sf::IntRect(menu_tex_width, 0, menu_tex_width, menu_tex_height), [=]() {
        const char *filters = "*.ser";
        std::string file_path = the_hud->default_file_path + "/blueprint.ser";
        const char *filename =
            tinyfd_openFileDialog("open File", file_path.c_str(), 1, &filters,
                                  "Serialized File", false);
        if (filename != NULL) {
  ...
  menu.addItem(
      menu_utility_texture,
      sf::IntRect(menu_tex_width * 2, 0, menu_tex_width, menu_tex_height),
      [=]() {
        const char *filters = "*.csv";
        std::string file_path = the_hud->default_file_path + "/blueprint.csv";
        const char *filename =
            tinyfd_saveFileDialog("export File", file_path.c_str(), 1, &filters,
